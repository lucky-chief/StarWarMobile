...
namespace Skill
{
    public sealed class Skill
    {
        ...
        private SkillTransition outTransition;

        public Skill(string name = "")
        {
            pause = true;
            Name = string.IsNullOrEmpty(name) ? GetHashCode().ToString() : name;
            states = new Dictionary<string, State>();
        }

        public Skill(XmlDocument xmlDoc)
        {
            ...
            for(int i = 0,size = states.Count; i < size; i++)
            {
                ...
                for(int j = 0; j < actions.Count; j++)
                {
                    ...
                    SkillAction action = (SkillAction)Reflection.GetExecutingAssembly().CreateInstance(className);
                ...
                for (int j = 0; j < transitions.Count; j++)
                {
                    ...
                    SkillTransition trans = new SkillTransition(transitionNode.Attributes["name"].Value,state.name,transitionNode.Attributes["toState"].Value);
                    ...
                    for(int k = 0; k < conditionsNodes.Count; k++)
                    {
                        ...
                        SkillCondition condition = (SkillCondition)Reflection.GetExecutingAssembly().CreateInstance(className);
        ...
        public void Update(float deltaTime)
        {
            ...
            foreach (KeyValuePair<string, SkillAction> action in currentState.actions)
            {
